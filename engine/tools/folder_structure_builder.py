...
classes_folders = glob.glob(os.path.join(DATA_PATH, 'EAN_13', '*'))

for idx, folder in enumerate(classes_folders):
    class_name = os.path.basename(folder)
    print(idx, class_name)

    train_target_folder = os.path.join(DATA_PATH, 'train', class_name)
    test_target_folder = os.path.join(DATA_PATH, 'test', class_name)
